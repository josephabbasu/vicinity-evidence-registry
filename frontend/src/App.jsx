...
const CERTAINTY_STYLES = {
  "Convergent design-supported evidence": "bg-emerald-50 text-emerald-800 border-emerald-200",
  "Design-supported evidence with important limitations": "bg-gold/15 text-amber-900 border-gold/30",
    ...
    <article className="card p-6">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-navy/10 pt-4">
        <span className="text-xs font-semibold text-slate-500">
          {study.country} · {study.publication_year || "Year needs verification"}
          {study.verification_flags?.length > 0 && (
            <span className="ml-2 rounded border border-amber-200 bg-amber-50 px-1.5 py-0.5 text-amber-700">
              {study.verification_flags.length} classification {study.verification_flags.length === 1 ? "note" : "notes"}
            </span>
          )}
        </span>
    ...
    <main className="page-shell py-12">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_320px]">
        ...
        <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="rounded-2xl border border-navy/10 bg-navy/3 p-5">
            <p className="text-xs font-bold uppercase tracking-wider text-navy mb-3">Review status</p>
            <div className="flex items-center gap-2">
              <span className={`rounded-full px-3 py-1 text-xs font-bold ${
                ...
                {study.verification_status === "verified" ? "Scientifically verified" : study.verification_status}
              </span>
            </div>
            {study.approval_status && (
              <p className="mt-2 text-xs text-slate-500">
                Registry approval: <span className="font-semibold">{study.approval_status}</span>
              </p>
            )}
          </div>

          {/* Scientific classification issues (need human resolution) */}
          {study.verification_flags?.length > 0 && (
            <div className="rounded-2xl border border-amber-300 bg-amber-50 p-5">
              <h2 className="text-sm font-bold text-amber-900">Classification notes</h2>
              <p className="mt-1 text-xs text-amber-700 leading-5">
                These require reviewer judgment before this record is used in a public claim.
              </p>
              <ul className="mt-3 space-y-2 text-xs leading-5 text-amber-900">
                {study.verification_flags.map((flag) => (
                  <li key={flag} className="border-t border-amber-200 pt-2 first:border-0 first:pt-0">{flag}</li>
                ))}
              </ul>
            </div>
          )}

          {/* Normalization notes (data-cleaning, not scientific classification) */}
          {study.normalization_notes?.length > 0 && (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
              <h2 className="text-sm font-bold text-slate-600">Data normalization notes</h2>
              <p className="mt-1 text-xs text-slate-500 leading-5">
                Metadata and formatting notes from the extraction workbook. These do not affect the scientific classification of this record.
              </p>
              <ul className="mt-3 space-y-2 text-xs leading-5 text-slate-600">
                {study.normalization_notes.map((note) => (
                  <li key={note} className="border-t border-slate-200 pt-2 first:border-0 first:pt-0">{note}</li>
                ))}
              </ul>
            </div>
          )}
